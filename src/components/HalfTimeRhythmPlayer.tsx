...
interface HalfTimeRhythmPlayerProps {
  mainPattern: boolean[];
  halfBeatPattern: boolean[];
  label: string;
  speedLevel?: number;
...
const HalfTimeRhythmPlayer = ({ 
  ...
  const track: Track = {
    ...
    sound: 'bass',
    pattern: mainPattern,
    halfPattern: halfBeatPattern,
    manuallyModified: new Array(8).fill(false),
    halfManuallyModified: new Array(8).fill(false)
  };

  const { isPlaying, currentBeat, currentHalfBeat, togglePlayback, playbackTime } = useRhythmPlayback({
    tracks: [track],
    speedLevels,
  ...
  const handleTogglePlayback = () => {
    if (isPlaying) {
      trackAudioPause('halftime_rhythm_player', label);
    } else {
      trackAudioPlay('halftime_rhythm_player', label);
    }
    togglePlayback();
  };
  ...
  return (
    <div className="bg-warm-brown/20 backdrop-blur-sm rounded-2xl p-6 border border-cream/20">
      ...
      {/* Visual rhythm display with main beats and half-time beats */}
      <div className="flex gap-2 justify-center">
        {mainPattern.map((isMainActive, index) => (
          <div key={index} className="flex items-center gap-1">
            {/* Main beat */}
            <div
              className={`
                w-12 h-12 rounded-lg border-2 flex items-center justify-center text-gray-700 font-bold
                ${isMainActive ? `${displayColor} border-gray-700` : 'bg-warm-brown/40 border-gray-400'}
                ${currentBeat === index && currentHalfBeat === 0 ? 'ring-4 ring-golden-yellow scale-110' : ''}
            ...
            {/* Half-time beat (smaller and round, except after the last main beat) */}
            {index < mainPattern.length - 1 && (
              <div
                className={`
                  w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs text-gray-600 font-medium
                  ${halfBeatPattern[index] ? `${displayColor} border-gray-700` : 'bg-warm-brown/30 border-gray-300'}
                  ${currentBeat === index && currentHalfBeat === 1 ? 'ring-2 ring-golden-yellow scale-125' : ''}
                  transition-all duration-200
                `}
              >
                +
...
export default HalfTimeRhythmPlayer;
