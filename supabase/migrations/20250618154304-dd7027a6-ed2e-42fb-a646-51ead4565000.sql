...
-- Update the function to count unique assignments by topic_index and assignment_key combination
...
BEGIN
  ...
  WITH daily_assignments AS (
    -- Group assignments by date and count distinct assignment combinations
    SELECT 
      ar.created_at::DATE as assignment_date,
      COUNT(DISTINCT (ar.topic_index, ar.assignment_key)) as daily_count
